  ...
  const portfolioItems = [
    {
      ...
      image: projet1, // ou images.projet1,
    ...
    },
    {
      ...
      image: projet2, // ou images.projet2,
    ...
    },
    {
      ...
      // image: 'https://images.unsplash.com/photo-1512941937669-90a1b58e7e9c?w=1200&h=800&fit=crop',
      image: projet3, // ou images.projet2,
      description: 'Cross-platform mobile apps for iOS and Android with React Native.',
      tags: ['React Native', 'Firebase'],
    },
    // {
    ...
    <div className={`min-h-screen transition-colors duration-300 relative ${theme === 'dark' ? 'dark' : ''}`}>
      ...
      <div className="relative z-10">
        ...
        <main>
            ...
            <div className="max-w-7xl mx-auto">
              ...
              <div className="grid md:grid-cols-3 gap-6">
                {workExperiences.map((exp, index) => (
                  <div
                    key={exp.id}
                    className="group relative bg-white/5 backdrop-blur-sm rounded-2xl p-6 border border-white/10 hover:border-white/30 transition-all duration-300 hover:transform hover:-translate-y-2"
                  >
          ...
          {/* Scroll Words Section with Dynamic Illustration */}
          {/* 
          <section className={`min-h-screen flex flex-col md:flex-row items-center px-8 md:px-20 py-12 md:py-0 ${snap ? 'scroll-snap-y' : ''}`}>
            <div className="w-full md:w-1/2">
              <div className="flex flex-row items-center justify-between md:block gap-4 mb-8">
                <h2 className="text-4xl md:text-6xl text-white dark:text-black font-bold">
                  Open for
                </h2>

                <div className="md:hidden flex items-center gap-3 bg-white/10 rounded-xl px-4 py-2 border border-white/20">
                  <span className="text-4xl animate-float">{scrollWords[activeWordIndex]?.illustration || '🎨'}</span>
                  <span className="text-white/80 text-sm">{scrollWords[activeWordIndex]?.text || 'design'}</span>
                </div>
              </div>
                ...
                <div className="flex gap-2 min-w-max">
                  {scrollWords.map((word, i) => (
                    <button
                      ...
                      className={`px-4 py-2 rounded-full text-sm font-medium transition-all whitespace-nowrap ${activeWordIndex === i
                        ? 'bg-white text-black scale-105'
                        : 'bg-white/10 text-white/70 hover:bg-white/20'
                        }`}
                      style={{
                        backgroundColor: activeWordIndex === i ? `oklch(65% 0.3 ${hueStart + (i * (hueEnd - hueStart) / (scrollWords.length - 1))})` : undefined,
                      }}
                    >
                      {word.text}
                    </button>
              ...
              <ul className="list-none font-semibold text-xl md:text-2xl hidden md:block">
                {scrollWords.map((word, i) => (
                  <li
                    key={i}
                    className={`scroll-word py-2 transition-all duration-300 cursor-pointer ${activeWordIndex === i ? 'scale-110 ml-4' : ''
                      }`}
                    style={{
            ...
            <div className="hidden md:flex md:w-1/2 items-center justify-center min-h-[400px] bg-white/5 backdrop-blur-sm rounded-2xl border border-white/10 md:ml-8">
              <div className="text-center">
                <div className="text-9xl mb-4 animate-float">
                  {scrollWords[activeWordIndex]?.illustration || '🎨'}
                </div>
                <p className="text-white/60 text-sm">
                  Currently focused on <span className="text-white font-semibold">{scrollWords[activeWordIndex]?.text || 'design'}</span>
                </p>
              </div>
            </div>
          </section> 
        ...
        <div className="fixed bottom-6 right-6 z-50">
          ...
          >
            <svg className="w-5 h-5 text-gray-900 dark:text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
          ...
          `}>
            <div className="space-y-4">
              <div>
                <label className="block text-xs opacity-70 mb-1 text-gray-900 dark:text-white">Theme</label>
                <select
                  value={theme}
                  onChange={(e) => setTheme(e.target.value)}
                  className="w-full p-2 rounded-lg bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-sm text-gray-900 dark:text-white"
                >
              ...
              <div className="flex items-center justify-between">
                <label className="text-sm text-gray-900 dark:text-white">Animate</label>
                <input
                  type="checkbox"
                  checked={animate}
              ...
              <div className="flex items-center justify-between">
                <label className="text-sm text-gray-900 dark:text-white">Snap Scroll</label>
                <input
                  type="checkbox"
                  checked={snap}
              ...
              <div>
                <label className="block text-xs opacity-70 mb-1 text-gray-900 dark:text-white">Hue Start: {hueStart}</label>
                <input
                  type="range"
                  min="0"
                  max="360"
              ...
              <div>
                <label className="block text-xs opacity-70 mb-1 text-gray-900 dark:text-white">Hue End: {hueEnd}</label>
                <input
                  type="range"
                  min="0"
                  max="360"
        ...
        */}
        ...
        {/* 
        {showArrow && (
          <div className="fixed left-[55%] top-[30%] md:top-auto md:bottom-8 md:left-auto md:right-4 text-sm opacity-70 z-50 pointer-events-none rotate-[-10deg] md:rotate-30 text-white dark:text-black">
